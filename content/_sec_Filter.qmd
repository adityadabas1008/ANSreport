...
In der invertierenden Summierschaltung werden mehrere Eingangsspannungen $V_1, V_2, \dots, V_m$ jeweils über gleiche Widerstände $R$ an den invertierenden Eingang eines Operationsverstärkers geführt. Der nichtinvertierende Eingang liegt auf Masse, und der Rückkopplungswiderstand ist $R_f$:

**Herleitung der Übertragungsfunktion**  
1. Knotenregel am invertierenden Eingang (Potenzial $V_- = 0$):

$$
\sum_{i=1}^{m} \frac{V_i - V_-}{R} + \frac{V_{\text{out}} - V_-}{R_f} = 0
\quad \Longrightarrow \quad
\sum_{i=1}^{m} \frac{V_i}{R} + \frac{V_{\text{out}}}{R_f} = 0.
$$

2. Auflösen nach $\frac{V_{\text{out}}}{V_i}$:

$$
\frac{V_{\text{out}}}{R_f} = - \sum_{i=1}^{m} \frac{V_i}{R}
\quad \Longrightarrow \quad
V_{\text{out}} = - R_f \sum_{i=1}^{m} \frac{V_i}{R}.
$$

3. Für gleiche Widerstände ($R_f = R$) vereinfacht sich das zu:
